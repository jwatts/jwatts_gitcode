...
CREATE TABLE HELP_R_RSC_SUBCAT (
  ...
  R_SUBCAT_ORDER number(10) NOT NULL,
  R_SUBCAT_ACTV_FLG number(3) DEFAULT NULL,
...
CREATE TABLE HELP_R_RSC_STS (
  ...
  R_STS_CD varchar2(255) DEFAULT NULL,
  R_STS_VAL varchar2(255) DEFAULT NULL,
  R_STS_ORD number(10) DEFAULT NULL,
  R_STS_ACTV_FLG number(3) DEFAULT NULL,
...
CREATE TABLE HELP_RSC (
  ...
  RSC_CNTCT_NAME varchar2(255) NOT NULL,
  RSC_CNTCT_PHONE varchar2(255) NOT NULL,
  RSC_CNTCT_EMAIL varchar2(255) NOT NULL,
...
-- Generate ID using sequence and trigger
CREATE SEQUENCE HELP_L_RSC_TAG_seq START WITH 1 INCREMENT BY 1;

CREATE OR REPLACE TRIGGER HELP_L_RSC_TAG_seq_tr
 BEFORE INSERT ON HELP_L_RSC_TAG FOR EACH ROW
 WHEN (NEW.RSC_ID IS NULL)
BEGIN
 SELECT HELP_L_RSC_TAG_seq.NEXTVAL INTO :NEW.RSC_ID FROM DUAL;
END;
/

/* ADD INDEXES AND CONSTRAINTS */
ALTER TABLE HELP_R_RSC_CAT
  ADD CREATE INDEX IDX_R_CAT_VAL ON HELP_R_RSC_CAT (R_CAT_VAL),
  ADD KEY `IDX_R_CAT_ACTV_FLG` (R_CAT_ACTV_FLG);
  
ALTER TABLE HELP_L_RSC_TAG
  ADD CREATE INDEX RSC_RSC_ID_FK ON HELP_L_RSC_TAG (RSC_ID),
  ADD KEY `R_TAG_R_TAG_ID_FK` (R_TAG_ID);

ALTER TABLE HELP_R_RSC_SUBCAT
  ADD CONSTRAINT RSC_SUBCAT_R_CAT_ID FOREIGN KEY (R_CAT_ID) REFERENCES HELP_R_RSC_CAT (R_CAT_ID);
  
ALTER TABLE HELP_RSC
  ADD CONSTRAINT RSC_R_CAT_ID_FK FOREIGN KEY (R_CAT_ID) REFERENCES HELP_R_RSC_CAT (R_CAT_ID),
  ADD CONSTRAINT RSC_R_STS_FK FOREIGN KEY (RSC_STS) REFERENCES `HELP_R_RSC_STS` (R_STS_ID),
  ADD CONSTRAINT RSC_R_SUBCAT_ID_FK FOREIGN KEY (R_SUBCAT_ID) REFERENCES `HELP_R_RSC_SUBCAT` (R_SUBCAT_ID),
  ADD CONSTRAINT RSC_R_TYPE_ID_FK FOREIGN KEY (R_TYPE_ID) REFERENCES `HELP_R_RSC_TYPE` (R_TYPE_ID);
